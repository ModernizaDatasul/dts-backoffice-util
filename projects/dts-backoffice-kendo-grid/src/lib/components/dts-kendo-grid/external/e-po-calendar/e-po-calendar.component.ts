...
import { isMobile, setYearFrom0To100, validateDateRange } from '../e-utils/e-util';
...
export class EpoCalendarComponent {
    ...
    @Input('p-date-input') inputEl: ElementRef;

    @Output('p-selected-dateChange') selectedDateChange = new EventEmitter<Date>();
    @Output('p-submit') submit = new EventEmitter<Date>();

    constructor(private ePoCalendarService: EpoCalendarService, private ePoCalendarLangService: EpoCalendarLangService) { }
    ...
    getWordMonth() {
        return this.ePoCalendarLangService.getWordMonth();
    }
    ...
    initializeLanguage() {
        this.ePoCalendarLangService.setLanguage(this.locale);
        this.displayWeedDays = this.ePoCalendarLangService.getArrayWeekDays();
        this.displayMonths = this.ePoCalendarLangService.getArrayMonths();
    }
    ...
    private updateDisplay(year: number, month: number) {
        const calendarArray = this.ePoCalendarService.monthDays(year, month);
