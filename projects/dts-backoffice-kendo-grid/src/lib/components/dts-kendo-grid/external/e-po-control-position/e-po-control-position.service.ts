...
@Injectable()
export class EpoControlPositionService {

    private arrowDirection: string;
    private customPositions: Array<string>;
    private custPosAlternative: Array<string>;
    ...
    setElements(element: ElementRef | HTMLElement, elementOffset: number, targetElement: ElementRef | HTMLElement,
        ...
        custPosAlternative?: Array<string>) {
        ...
        this.custPosAlternative = custPosAlternative;
    ...
    private adjustCustomPosition(position: string) {
        ...
        let sidesCount = 0;
        while (sidesCount++ < this.customPositions.length) {
        ...
        if (!isPosOk && this.custPosAlternative && this.custPosAlternative.length) {
            sidesCount = 0;
            while (sidesCount++ < this.custPosAlternative.length) {
                if (this.overflowMain(this.adjustPosAlternative(position))) {
                    position = this.nextPosition(position, this.custPosAlternative);
                    this.elementPosition(position);
                } else {
                    break;
                }
    ...
    private adjustPosAlternative(position: string) {
    ...
    private overflowAllSides(position: string) {
        ...
        switch (position) {
            ...
            case 'top-right': return overflows.top || this.getDirectByCornerAligned('right', overflows);
            case 'top-left': return overflows.top || this.getDirectByCornerAligned('left', overflows);
            case 'right': return overflows.right || overflows.top || overflows.bottom;
            case 'right-top': return this.getDirectByCornerAligned('right', overflows) || overflows.top;
            case 'right-bottom': return this.getDirectByCornerAligned('right', overflows) || overflows.bottom;
            case 'bottom': return overflows.bottom || overflows.right || overflows.left;
            case 'bottom-right': return overflows.bottom || this.getDirectByCornerAligned('right', overflows);
            case 'bottom-left': return overflows.bottom || this.getDirectByCornerAligned('left', overflows);
            case 'left': return overflows.left || overflows.top || overflows.bottom;
            case 'left-top': return this.getDirectByCornerAligned('left', overflows) || overflows.top;
            case 'left-bottom': return this.getDirectByCornerAligned('left', overflows) || overflows.bottom;
        }
    }
    ...
    private overflowMain(position: string) {
        ...
        switch (position) {
            ...
            case 'right': return this.getDirectByCornerAligned('right', overflows);
            case 'bottom': return overflows.bottom;
            case 'left': return this.getDirectByCornerAligned('left', overflows);
        }
    }

    private getDirectByCornerAligned(position: string, overflows: any) {
        switch (position) {
            case 'right': return this.isCornerAligned ? overflows.left : overflows.right;
            case 'left': return this.isCornerAligned ? overflows.right : overflows.left;
        }
    }
